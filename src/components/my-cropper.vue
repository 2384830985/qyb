<template>
    <el-dialog :visible="isShow" title="图片剪裁" :close-on-click-modal="false" @open="openCropper" @close="closeCropper">
        <el-row>
            <el-col :span="16">
                <div class="cropper">
...
<script>
    import {upLoadPic} from '@/api/event'
        ...
        methods: {
            ...
            crop() {
                ...
                cas.toBlob(blob => {
                    ...
                    data.append('file', blob, `${_id}.jpg`)
                    upLoadPic({
                        data: data
                    }).then(res => {
                        this.isUpload = false
                        if (res.resultCode === 0) {
                            this.updateData = Object.assign(this.cropperData, {
                                url: res.result.url
                            })
